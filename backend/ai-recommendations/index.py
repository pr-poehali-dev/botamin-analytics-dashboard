...
def handler(event: dict, context) -> dict:
    ...
    # ── 8. ТОП ПРИЧИН ОТКАЗОВ — с B2B контекстом ─────────────────────────
    if top_fail:
        # Определяем главный тип отказа
        top_reason = top_fail[0][0] if top_fail else ''
        is_secretary_block = any(k in top_reason.lower() for k in
            ['секретар', 'почту', 'пришлите', 'не нужно', 'не интересно', 'занят', 'перезвоните'])

        if is_secretary_block:
            recs.append({
                'id': 'fail_secretary',
                'priority': 'high',
                'category': 'Проход секретаря',
                'icon': 'DoorClosed',
                'title': 'Главный барьер — секретарский фильтр',
                'problem': f'Большинство отказов происходит ещё до выхода на ЛПР. '
                           f'Типичный ответ: «{top_reason[:60]}». '
                           f'Это стандартный B2B барьер, но с ним можно работать.',
                'action': 'Отработайте 3 техники прохода секретаря: '
                          '1) Метод «своего» — говорить уверенно как внутренний сотрудник; '
                          '2) Называть имя ЛПР по имени-отчеству; '
                          '3) Ссылаться на предыдущий контакт или договорённость.',
                'metric': f'{failure_count} отказов, из них большинство — секретарский барьер',
                'target': 'Цель: увеличить выход на ЛПР с текущего до 15–20%',
            })
        else:
            ...
            recs.append({
                'id': 'fail_reasons',
                'priority': 'medium',
                'category': 'Причины отказов',
                'icon': 'XCircle',
                'title': 'Повторяющиеся причины отказов — нужны скрипты отработки',
                'problem': f'Чаще всего отказывают по причинам: {reason_text}. '
                           f'Это повторяется — значит операторы не готовы к этим ситуациям.',
                'action': 'Создайте карточку с 3 вариантами ответа на каждую из топ-причин. '
                          'Проведите 30-минутную ролевую игру по этим сценариям — это быстро даст результат.',
                'metric': f'{failure_count} отказов из {total}',
                'target': 'Снизить отказы по топ-причине на 30% за 2 недели',
            })

    # ── 9. ОЦЕНКА ОПЕРАТОРОВ 2.8/10 — особый случай ──────────────────────
    # Низкая оценка при холодных звонках может быть нормой если это оценивается жёстко
    # Но 2.8 — это сигнал на базовые ошибки
    if avg_score < 4:
        recs.append({
            'id': 'score_basics',
            'priority': 'high',
            ...
            'icon': 'AlertTriangle',
            'title': f'Оценка {avg_score}/10 — фокус на базовых ошибках',
            'problem': f'При холодных B2B-звонках норма — 5–6/10. '
                       f'Оценка {avg_score} говорит о базовых проблемах: '
                       f'нет чёткого представления, нет оффера, сдаётся при первом «нет».',
            'action': '1) Убедитесь что оператор представляется полностью (имя + компания + 1 фраза о ценности). '
                      '2) Проверьте есть ли конкретный вопрос в конце каждого звонка. '
                      '3) Послушайте 3 худших звонка вместе с командой — обычно проблема одна и та же.',
            'metric': f'{avg_score}/10 средняя оценка',
            'target': 'Первый шаг: поднять до 5/10 за месяц',
        })
    ...
    if top_success and success_count > 0:
        ...
        recs.append({
            'id': 'success_scale',
            'priority': 'medium',
            'category': 'Что работает',
            'icon': 'Trophy',
            'title': 'Есть успешные звонки — разберите их детально',
            'problem': f'В успешных звонках общий фактор: «{factor_text[:80]}». '
                       f'{success_count} из {total} — это ваш рабочий паттерн.',
            'action': 'Запишите разбор каждого успешного звонка: '
                      'на какой секунде произошёл перелом, что именно сказал оператор, '
                      'как отреагировал клиент. Сделайте это шаблоном.',
            'metric': f'{success_count} успешных из {total}',
            'target': 'Довести долю по паттерну успешных до 50% звонков',
        })

    # ── 11. КЛЮЧЕВЫЕ ФРАЗЫ — адаптация оффера ────────────────────────────
    if top_phrases and len(top_phrases) >= 3:
        phrases_text = ', '.join([f'«{p}»' for p, _ in top_phrases[:3]])
        recs.append({
            'id': 'client_phrases',
            'priority': 'low',
            'category': 'Язык клиента',
            'icon': 'MessageSquare',
            'title': 'Клиенты сами подсказывают как говорить с ними',
            'problem': f'Клиенты используют формулировки: {phrases_text}. '
                       f'В B2B это особенно ценно — каждая отрасль говорит на своём языке.',
            'action': 'Внедрите эти формулировки в скрипт открытия звонка. '
                      'Вместо «мы помогаем привлекать клиентов» — говорите словами самих клиентов. '
                      'Это резко повышает доверие с первых секунд.',
            'metric': f'{len(phrases_client)} фраз проанализировано',
            'target': 'Протестировать новый оффер с языком клиента на 20 звонках',
        })

    # ── 12. ОБЩАЯ РЕКОМЕНДАЦИЯ если мало данных ───────────────────────────
    if total < 50:
        recs.append({
            'id': 'more_data',
            'priority': 'low',
            ...
            'problem': 'Для статистически значимых выводов по холодным B2B-звонкам '
                       'нужно минимум 100–150 звонков. Сейчас данных мало — тренды могут быть случайными.',
            'action': 'Запустите авто-режим (кнопка АВТО) чтобы транскрибировать и проанализировать '
                      'все доступные звонки. Чем больше данных — тем точнее рекомендации.',
            'metric': f'{total} из 708 звонков с записью проанализировано',
            'target': 'Проанализировать 150+ звонков для надёжных выводов',
        })

    # Сортируем: high → medium → low, максимум 8 рекомендаций
    priority_order = {'high': 0, 'medium': 1, 'low': 2}
    recs.sort(key=lambda x: priority_order[x['priority']])
    recs = recs[:8]  # не перегружаем
